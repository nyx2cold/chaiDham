...
function LiveOrderTracker({ order }: { order: Order }) {
    ...
    return (
        <div className={`relative rounded-2xl overflow-hidden border transition-all duration-300
                    ...
                    {STATUS_STEPS.map((step, i) => {
                        ...
                        return (
                            <div key={step.key}>
                                ...
                                {i < STATUS_STEPS.length - 1 && <div className={`ml-[22px] w-px h-2.5 ${isDoneStep ? "bg-emerald-500/20" : "bg-white/[0.05]"}`} />}
                            </div>
                        );
                    })}
                </div>
            ...
            <p className="px-4 pb-3 text-[10px] text-zinc-600">{formatDate(order.createdAt)} · {formatTime(order.createdAt)}</p>
        </div>
    );
}
...
function AccountForm({
    ...
    const labelCls = "block text-[10px] font-bold text-zinc-600 uppercase tracking-[0.12em]";
        ...
        <div className="p-5 space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5 sm:col-span-2">
                    ...
                    <div className="h-10 px-3.5 rounded-xl text-sm text-zinc-500 flex items-center bg-white/[0.02] border border-white/[0.05] cursor-not-allowed select-none">
                        {memberSince}
                    </div>
                </div>
            </div>
            ...
            <button onClick={handleSave} disabled={saving}
                className="w-full py-3 rounded-xl text-sm font-black text-zinc-950 bg-amber-500 hover:bg-amber-400 shadow-[0_4px_20px_rgba(245,158,11,0.25)] hover:shadow-[0_4px_28px_rgba(245,158,11,0.4)] active:scale-[0.99] disabled:opacity-60 disabled:cursor-not-allowed transition-all duration-150">
                ...
                <div className="relative rounded-2xl overflow-hidden mb-3 bg-white/[0.04] backdrop-blur-xl border border-white/[0.08] shadow-[0_8px_40px_rgba(0,0,0,0.4),inset_0_1px_0_rgba(255,255,255,0.08)]">
                    ...
                    <div className="relative flex items-center gap-5 p-5 flex-wrap">
                        <div className="relative shrink-0">
                            <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-amber-400 to-amber-600 flex items-center justify-center text-2xl font-black text-zinc-950 shadow-[0_0_0_3px_rgba(245,158,11,0.2),0_0_24px_rgba(245,158,11,0.2)]">
                                {getInitials(displayName)}
                            </div>
                        ...
                        <button onClick={() => signOut({ callbackUrl: "/sign-in" })}
                            className="shrink-0 flex items-center gap-2 px-3 py-2 rounded-xl text-xs font-bold bg-red-500/[0.07] text-red-400 border border-red-500/15 hover:bg-red-500/15 hover:border-red-500/25 active:scale-95 transition-all duration-150">
                            <LogOut className="h-3.5 w-3.5" /> Sign out
                        </button>
                    </div>
                </div>
                    ...
                    <div className="relative p-5">
                        ...
                        <div className="mb-4">
                            ...
                            <div className="h-1.5 rounded-full bg-white/[0.06] overflow-hidden">
                                <div className="h-full rounded-full bg-gradient-to-r from-amber-500 to-amber-400 shadow-[0_0_10px_rgba(245,158,11,0.4)] transition-all duration-700 ease-out"
                    ...
                    <div>
                        ...
                        {ordersTab === "live" && (
                            loading ? (
                                ...
                                <div className="space-y-3">{liveOrders.map((o) => <LiveOrderTracker key={o._id} order={o} />)}</div>
                            )
                        )}

                        {ordersTab === "history" && (
                            <div className="rounded-2xl overflow-hidden bg-white/[0.03] backdrop-blur-xl border border-white/[0.07] shadow-[0_4px_24px_rgba(0,0,0,0.25),inset_0_1px_0_rgba(255,255,255,0.05)]">
                                <div className="flex items-center gap-2.5 px-5 py-4 border-b border-white/[0.06]">
                                    <p className="text-sm font-bold text-white">Order History</p>
                                    {historyOrders.length > 0 && (
                                        <span className="px-1.5 py-0.5 rounded-md text-[10px] font-bold bg-amber-500/12 text-amber-400 border border-amber-400/20">{historyOrders.length}</span>
                                    )}
                                </div>
                                {loading ? (
                                                    ...
                                                    return (
                                                        <tr key={order._id} className={`border-b border-white/[0.03] transition-colors hover:bg-white/[0.025] ${i % 2 !== 0 ? "bg-white/[0.015]" : ""}`}>
                                                            ...
                                                            <td className="px-4 py-3 text-zinc-400 max-w-[160px]">
                                                                <span className="block truncate">{order.items.map((it) => `${it.name} ×${it.quantity}`).join(", ")}</span>
                                                            </td>
                                                            <td className="px-4 py-3 whitespace-nowrap">
                                                                <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${order.type === "dine-in" ? "bg-sky-500/10 text-sky-400 border-sky-500/20" : "bg-violet-500/10 text-violet-400 border-violet-500/20"}`}>
                                                                    {order.type === "dine-in" ? "Dine In" : "Takeaway"}
                                                                </span>
                                                            </td>
                                                            <td className="px-4 py-3 whitespace-nowrap">
                                                            ...
                                                            <td className="px-4 py-3 font-black text-white whitespace-nowrap">₹{order.total}</td>
                                                        </tr>
                                                    );
                                                })}
                                            </tbody>
