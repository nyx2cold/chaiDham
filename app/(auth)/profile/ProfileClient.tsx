...
import {
    ...
    Sparkles, CircleCheck, CircleX, Loader2,
    ChevronLeft,
} from "lucide-react";
...
const HISTORY_PAGE_SIZES = [5, 10, 20] as const;
...
function Pagination({
    ...
    return (
        <div className="flex items-center gap-1">
            ...
            {pages.map((p, idx) =>
                p === "..." ? (
                    <span key={`e-${idx}`} className="text-zinc-600 text-xs px-1">…</span>
                ) : (
                    <button
                        key={p}
                            ...
                            <div key={step.key}>
                                <div className={`flex items-center gap-3 px-3 py-2 rounded-xl border transition-all duration-300
                                    ...
                                    <StepIcon className={`h-4 w-4 shrink-0
                                        ${isActiveStep ? "text-amber-400" : isDoneStep ? "text-emerald-400" : "text-zinc-600"}`} />
...
// ── History Table ─────────────────────────────────────────────────────────────
function HistoryTable({ orders, loading }: { orders: Order[]; loading: boolean }) {
    const [page, setPage] = useState(1);
    const [pageSize, setPageSize] = useState<typeof HISTORY_PAGE_SIZES[number]>(10);
    ...
    const paginated = orders.slice((safePage - 1) * pageSize, safePage * pageSize);

    return (
        <div className="relative rounded-2xl overflow-hidden">
            {/* Glass background — identical to OrdersTable */}
            ...
            <div className="relative">
                ...
                <div className="flex items-center justify-between gap-3 px-5 py-4
                    border-b border-white/[0.06] bg-gradient-to-r from-white/[0.04] to-transparent flex-wrap">
                    <div className="flex items-center gap-2">
                        <p className="text-sm font-bold text-white tracking-tight">Order History</p>
                        {orders.length > 0 && (
                            <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md
                                text-[10px] font-bold text-amber-300 bg-amber-500/15 border border-amber-400/25">
                    ...
                    {orders.length > 0 && (
                        <div className="flex items-center gap-1.5 px-2 h-8 rounded-xl
                ...
                {loading ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-16 text-center">
                        <div className="h-12 w-12 rounded-2xl mb-3 bg-white/[0.04] border border-white/[0.08]
                            flex items-center justify-center">
                            <Package className="h-5 w-5 text-zinc-600" />
                        </div>
                        <p className="text-zinc-500 text-sm font-medium">No completed orders yet</p>
                        <p className="text-zinc-700 text-xs mt-1">Your order history will appear here</p>
                    </div>
                ) : (
                    <>
                                    ...
                                    <tr className="border-b border-white/[0.05]">
                                        {["Order", "Items", "Type", "Status", "Date", "Total"].map((h) => (
                                            <th key={h} className="px-4 py-3 text-left
                                                text-[10px] font-bold text-zinc-600 uppercase tracking-widest whitespace-nowrap">
                                                {h}
                                            </th>
                                        ))}
                                    </tr>
                                            ...
                                            <tr key={order._id}
                                                ...
                                                <td className="px-4 py-3 whitespace-nowrap">
                                                    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg
                                                        text-[10px] font-bold border ${s.bg} ${s.text} ${s.border}`}>
                                                ...
                                                <td className="px-4 py-3 whitespace-nowrap">
                                                    ...
                                                    <p className="text-[10px] text-zinc-600">{formatTime(order.createdAt)}</p>
                                                </td>
                            ...
                            <div className="flex items-center justify-between px-5 py-3
                                ...
                                <p className="text-[11px] text-zinc-500">
                                    ...
                                    <span className="text-zinc-300 font-semibold">
                                        {(safePage - 1) * pageSize + 1}–{Math.min(safePage * pageSize, orders.length)}
                                    </span>{" "}
                                    of{" "}
                                    <span className="text-zinc-300 font-semibold">{orders.length}</span> orders
                                </p>
                                <Pagination
...
function AccountForm({
    ...
    const inputCls = "w-full h-10 px-3.5 rounded-xl text-sm text-white placeholder:text-zinc-600 bg-white/[0.05] border border-white/[0.09] focus:outline-none focus:border-amber-500/50 focus:bg-white/[0.08] transition-all duration-200";
    const labelCls = "block text-[10px] font-bold text-zinc-600 uppercase tracking-[0.12em] mb-1.5";
        ...
        <div className="p-5 space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="10-digit mobile" className={inputCls} />
                </div>
            ...
            <button onClick={handleSave} disabled={saving}
                className="w-full py-3 rounded-xl text-sm font-black text-zinc-950
                    ...
                    shadow-[0_4px_20px_rgba(245,158,11,0.25)] hover:shadow-[0_4px_28px_rgba(245,158,11,0.4)]
                    active:scale-[0.99] disabled:opacity-60 disabled:cursor-not-allowed transition-all duration-150">
        ...
        return (
            <div className="min-h-screen bg-zinc-950 text-white">
                ...
                <div className="relative mx-auto max-w-xl px-4 py-8 pb-16">
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                                    ...
                                    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full
                                        text-[10px] font-black bg-red-500/10 text-red-400 border border-red-500/25
                    ...
                    <div className="rounded-2xl overflow-hidden bg-white/[0.04] backdrop-blur-xl border border-white/[0.08]
                        shadow-[0_4px_24px_rgba(0,0,0,0.3),inset_0_1px_0_rgba(255,255,255,0.07)]">
    ...
    return (
        <div className="min-h-screen bg-zinc-950 text-white">
            ...
            <div className="relative mx-auto max-w-3xl px-4 py-8 pb-16">
                        ...
                        <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 flex-wrap mb-1">
                                ...
                                <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full
                                    text-[11px] font-bold border ${tier.bg} ${tier.border} ${tier.color}`}>
                                    <TierIcon className="h-3 w-3" /> {tier.name}
                                </span>
                                {liveOrders.length > 0 && (
                                    <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full
                                        text-[11px] font-bold bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
                ...
                <div className="grid grid-cols-3 gap-2.5 mb-3">
                    {[
                        ...
                        <div key={s.label}
                            ...
                            <div className="pointer-events-none absolute -top-4 -right-4 w-16 h-16 rounded-full
                                bg-amber-500/[0.05] blur-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                ...
                {/* ── Brownie Points ── */}
                <div className="relative rounded-2xl overflow-hidden mb-3
                    bg-white/[0.04] backdrop-blur-xl border border-amber-500/[0.12]
                    shadow-[0_4px_24px_rgba(0,0,0,0.3),inset_0_1px_0_rgba(255,255,255,0.07)]">
                    ...
                    <div className="relative p-5">
                        <div className="flex items-start justify-between gap-4 mb-4">
                            <div>
                                ...
                                <p className="text-[11px] text-zinc-600 mt-1.5">1 pt per ₹10 · 1 pt = ₹1 discount</p>
                            </div>
                ...
                {/* ── Main tabs ── */}
                <div className="flex gap-1 p-1 rounded-xl mb-3 bg-white/[0.03] border border-white/[0.07]">
                    {(["orders", "account"] as const).map((tab) => (
                        <button key={tab} onClick={() => setActiveTab(tab)}
                            className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-lg
                                text-xs font-bold uppercase tracking-wider transition-all duration-200 active:scale-[0.98]
                                ${activeTab === tab
                ...
                {activeTab === "orders" && (
                    <div>
                        {/* Sub-tabs */}
                        <div className="flex gap-1 p-1 rounded-xl mb-3 bg-white/[0.02] border border-white/[0.06]">
                            {(["live", "history"] as const).map((t) => {
                                const count = t === "live" ? liveOrders.length : historyOrders.length;
                                const isActive = ordersTab === t;
                                const isLiveWithOrders = t === "live" && liveOrders.length > 0;
                                return (
                                    <button key={t} onClick={() => setOrdersTab(t)}
                                        className={`flex-1 flex items-center justify-center gap-2 py-1.5 rounded-lg
                                            text-[11px] font-bold uppercase tracking-wider transition-all duration-200 active:scale-[0.98]
                                            ${isActive
                                                ? isLiveWithOrders
                                        ...
                                        {isLiveWithOrders && <span className="w-1.5 h-1.5 rounded-full bg-amber-500 animate-pulse" />}
                                        ...
                                        {count > 0 && (
                                            <span className={`flex items-center justify-center h-4 w-4 rounded-full text-[9px] font-black
                                                ${isActive
                                                    ? isLiveWithOrders ? "bg-amber-500/20 text-amber-400" : "bg-white/15 text-white"
                        ...
                        {ordersTab === "live" && (
                            loading ? (
                                <div className="rounded-2xl bg-white/[0.03] border border-white/[0.07] p-10 text-center">
                        ...
                        {/* History — uses HistoryTable component */}
                        {ordersTab === "history" && (
                            <HistoryTable orders={historyOrders} loading={loading} />
                        )}
                    </div>
                )}
